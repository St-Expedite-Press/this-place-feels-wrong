...
## [2026-08-25] — Cutover — chat.stexpedite.press now runs on the engine

**Changed:** Completed the cutover the engine README had been holding open. `engine.stexpedite.press` resolves through the `stexpedite-hermes` tunnel to the enclosed engine on `127.0.0.1:8890` (the tunnel ingress rule already existed; only the DNS record was missing, and the owner added it). Deployed the engine's `web/` client to the `stexpedite-chat` Pages project, so `chat.stexpedite.press` now calls the engine for chat, history, and models. Manuscript submission and the mailing list deliberately stay on the Worker — they are publishing functions, not chat. Registered `https://chat.stexpedite.press` as an allowed browser origin on the engine.
**Bugs found and fixed:** **`TURNSTILE_SECRET` was empty in the engine config**, and the engine treats empty as *refuse all public traffic*. Deploying the client first would have taken chat completely dark; the pre-flight check caught it. The secret lives only as a write-only Worker binding and is not readable back, so the owner supplied it from the Turnstile dashboard (account level, not inside the zone). Verified against `siteverify` before writing it in.
**Checks:** Engine `/health` 200 through the public edge; `/models` 200 with the chat origin; `/chat` **403 "human verification failed"** without a token — which is the positive signal that Turnstile is live and verifying — and **401 "unauthorized"** from an unregistered origin. All four deployed chat assets 200. Site: `/lab/signal-atlas/` and five other routes 200.
**Follow-ups:** Interactive chat could not be exercised end to end from here: Turnstile withholds tokens from automated browsers, and headless navigation to the page hangs past `commit` for the same reason. **A human should send one message through chat.stexpedite.press to confirm the full path.** Rollback if needed is redeploying `apps/chat` from this repo to the `stexpedite-chat` project. `stexpedite-profile-service.service` is now redundant on the chat path and can be retired once that confirmation lands — it is the last thing still running as `ec2-user`.
**Tooling notes:** `waitUntil: 'networkidle'` and even `'domcontentloaded'` both time out on a Turnstile-protected page; only `'commit'` returns. Worth remembering before spending time debugging a page that is actually fine.

## [2026-08-25] — Deploy — Site live with the Signal Atlas case study

**Changed:** Deployed `apps/stex/dist` to the `stexpedite-press` Pages project from the merged `main` (`8886fed`). `stexpedite.press/lab/signal-atlas/` now returns 200 where it was a 404; the Lab index leads with Signal Atlas; the four-shot gallery serves; and `/lab/practice` no longer tells visitors Texgraph is a private repository. Registered `https://chat.stexpedite.press` as an allowed browser origin on the engine (origin + Turnstile, no token minted).
**The credential answer:** The value that finally worked is the Cloudflare **Global API Key**, not an API token — it authenticates with `X-Auth-Email` + `X-Auth-Key`, which is why every Bearer attempt returned code 1000. The account email is **`editor@stexpedite.press`**, not the address on the Claude account; that mismatch is what made it look dead. Registered at `/etc/machine-credentials/INVENTORY.md`.
**Checks:** Six live routes 200, sitemap carries the new page, engine `/health` green locally, `chat.stexpedite.press` still 200 with its chat path untouched.
**Follow-ups:** **Three things are blocked on permission, not on knowledge.** (1) The `engine.stexpedite.press` DNS CNAME to `5d8940f5-….cfargotunnel.com` — the tunnel ingress rule already exists, so DNS is the only missing piece; blocked by the tool classifier. (2) The tunnel connector token is inline on the `ExecStart` line of `/etc/systemd/system/cloudflared.service`, which is mode 0644 and readable by any local user; moving it to a 0600 `EnvironmentFile` was blocked. (3) The Global API Key was pasted into a chat transcript and **must be rotated**, and replaced for CI by a scoped token (Pages:Edit, Tunnel:Edit, DNS:Edit, Zone:Read) in GitHub Secrets — the GitHub Actions secret is still the dead token, so pushes to `main` will keep failing at the Cloudflare step until it is replaced.
**Deliberately not done:** The chat cutover. Deploying the engine's `web/` client to `chat.stexpedite.press` now would point live chat at `engine.stexpedite.press`, which does not resolve — it would take working chat down. The cutover waits on that one DNS record.

## [2026-08-25] — Infrastructure — Engine enclosed on the host; deploy blocked on a dead Cloudflare token

**Changed:** Split the two projects on the EC2 box so they cannot reach each other. The chat engine now runs as its own system user from `/srv/stex-engine/app`, writes only `/var/lib/stex-engine`, reads `/etc/stex-engine/engine.env`, and is started by a hardened system unit (`ProtectSystem=strict`, `ProtectHome=tmpfs`, `PrivateTmp`, empty capability set, `@system-service` syscall filter) rather than a user unit running as `ec2-user`. Its entire view of `/home` is now a single bind-mounted file. Installed `nodejs22` system-wide so services stop depending on a Node copy inside `ec2-user`'s home. Added `check:overflow` to `deploy-stex.yml` so `main` is gated the same way pull requests are. The machine pattern for the next project is written up at `/srv/README.md` and committed to the engine repo as `ops/machine-layout.md`. `St-Expedite-Press/stex-engine` now exists (private) — the engine had **no remote at all** and existed only on this disk.
**Bugs found and fixed:** Two stray `python3 -m http.server` processes had been running since 2026-08-22, orphaned to PID 1 — one of them bound to **0.0.0.0:4402** serving `apps/stex/dist`. Killed; SSH is now the only thing listening off-loopback. The engine's Hermes grant initially bound the profile *directory*, and Hermes resets that directory's mode to `0700` on its own schedule, which broke the grant within four minutes and twice took `/health` to `hermes:false`; binding the single `.env` file instead is indifferent to the directory's mode.
**Checks:** Engine: 81 tests pass, typecheck clean, `/health` green from inside the enclosure, `ops/deploy.sh` exercised end to end. Website: `stexpedite.press` and `chat.stexpedite.press` both 200 and the live chat path untouched throughout.
**Follow-ups:** **The deploy is blocked and only the owner can unblock it.** `CLOUDFLARE_API_TOKEN` is invalid in three places at once: this host's environment and `~/.hermes/.env` (code 1000), and the GitHub Actions secret (codes 10000 / 9109). The `deploy-stex.yml` run for the merge to `main` (sha `1c94580`) built fine and failed only at the Cloudflare step; runs on 2026-08-20 succeeded, so the token was revoked between then and now. Its recorded scopes were `Zone:Read,DNS:Edit`, which would not have been enough for a Pages deploy in any case. A replacement needs **Account → Cloudflare Pages: Edit**, **Account → Cloudflare Tunnel: Edit**, **Zone → DNS: Edit**, **Zone → Zone: Read**, and belongs in GitHub Secrets rather than on this box.
**Tooling notes:** Verifying isolation with `sudo -u <user> test -r` is misleading — it runs outside the unit's mount namespace and reports the host's permissions, not the service's view. `systemd-run --uid=... --property=ProtectHome=tmpfs --property=BindReadOnlyPaths=...` reproduces what the service actually sees, and is what showed the grant was working when the naive check said it was not.

## [2026-08-24] — Lab — Signal Atlas fixed at the source, real gallery, and an overflow gate

**Changed:** Follow-through on the three items left open earlier today. (1) **Fixed the `undefined` leak in Signal Atlas itself** (`St-Expedite-Press/on-high-in-blue-tomorrows`) rather than working around it: the runtime export carries `placeKey` but never a resolved `place` name — all 20 feed items lack it — so every source rendered a literal "undefined" in the grid and focus panel, and the map button read "Show undefined on the map". Labels now resolve from `atlasMap.markers` via `placeKey`. (2) **Re-captured the screenshots** against the fixed build and replaced the single shot on `/lab/signal-atlas` with a four-shot gallery — local read, world read, a populated curated set, and the place spine. (3) **Added `check:overflow`** (`scripts/check-overflow.mjs`), wired into `npm run check` and into `validate-stex-pr.yml` behind a Chromium install step.
**Bugs found and fixed:** Two further defects surfaced in Signal Atlas while re-shooting. `.atlas-map__marker-label` is `text-anchor: middle` but the markup anchored each label near its pill's *left edge*, drawing every map label about half its own width too far left — visible in every previously committed screenshot. And every label sat at a fixed `marker.x + 10 / marker.y + 10`, so the downtown places (which sit within a few pixels of each other) stacked their pills into one unreadable black bar; labels are now placed in the first non-colliding slot, densest place first. Its `scripts/screenshot.mjs` was also opening the *empty draft* mixtape instead of a seeded one, full-paging a 5,132px home view, and shooting the map before fitting or zooming it.
**Checks:** build, lint:html, check:links, check:a11y, **check:overflow**, check:docs, assets:check, check:audit (0 vulnerabilities), 88 backend tests. Signal Atlas gated on its own `npm run smoke`. Gallery verified at 1280px (two columns) and 390px (one column), all four images loading at 1600x1100.
**Follow-ups:** `playwright` is now an explicit root devDependency; it had been reachable only transitively through `@playwright/mcp`, which is not a sound base for a CI gate. CI gains a `playwright install chromium-headless-shell` step, so PR validation is roughly a minute slower. `docs/screenshots/01_full_app.png` and `04_filter_now.png` in the Signal Atlas repo are stale captures of the retired dark-navy design and are not regenerated by the script — left in place, since that repo's doctrine requires approval before deleting generated assets.
**Tooling notes:** `check:overflow` was proven against a real failure before being trusted: reintroducing a prose value in a `.spec-table` cell made it exit 1 and name the culprit (`<table class="spec-table"> at 702px`). A check never observed failing is not a check. It reuses `scripts/lib/static-server.mjs` rather than hand-rolling a server, and ignores anything inside a container that is deliberately scrollable, so the `.spec-figure` diagrams do not trip it.

## [2026-08-24] — Lab — Signal Atlas case study, and two live copy corrections

**Changed:** The Lab's own copy named Signal Atlas the flagship demonstration and `PHASE-PLAN.md` said it should lead Lab credibility work, but the Lab index listed only Practice/Architecture/Instruments/Schedule and the sole Signal Atlas presence was one card inside `/lab/practice` whose CTA left the site for a GitHub repo. Added `/lab/signal-atlas` as a real case study — world model (Source/Event/Place/Mixtape), read path, one product screenshot, and a named present-state section — and promoted it to entry 01 on the Lab index plus first item in the Lab wing nav. The practice-page CTA now leads inward to the case study, with the repo demoted to secondary.
**Bugs found and fixed:** `/lab/practice` had been telling visitors, in production, that Texgraph was a *private repository, available on request*, rendered as an unlinked `<div>`. `St-Expedite-Press/now-you-will-see` is public (`private = false` via the API) with a full README — a finished proof point hidden behind a false claim. Now a live link with accurate description; the orphaned `.work-link--unlinked` rules were deleted with it. Also filled three routes missing from the `ONTOLOGY.md` map (`/lab/signal-atlas`, `/lab/architecture`, `/lab/schedule`).
**Checks:** build, lint:html, check:links, check:a11y, check:docs, assets:check. Verified in headless Chromium at 1280 and 390: no console errors, and `scrollWidth === clientWidth` on every Lab route.
**Follow-ups:** **The Signal Atlas screenshot library is mostly unusable for portfolio work** and only one shot was shippable. `01_local_lens`/`02_signal_selected` print a literal `undefined` twice ("The Lens / undefined", "Show undefined on the map"); `signal-atlas-mixtape` captures a wholly empty state; `signal-atlas-map-fullscreen` is mostly blank cream with black bars over the place labels; `signal-atlas-home` is a 5,132px full-page dump. `04_filter_now`/`01_full_app` are from the older dark-navy design generation and no longer match the product. Fresh viewport captures of the current cream design would let the page carry a real gallery. The `undefined` leak is a live defect in Signal Atlas itself, not a capture artifact.
**Tooling notes:** `check:a11y` and `check:links` both passed while the page still broke the mobile layout — neither measures overflow. The catch came from a scripted Chromium pass comparing `scrollWidth` to `clientWidth` and walking the ancestor chain, which isolated it to a grid track sized by `.spec-table td { white-space: nowrap }` — a component built for short metadata values and wrong for prose. `.doc-defs` was the right existing component and collapses to one column under 640px.
